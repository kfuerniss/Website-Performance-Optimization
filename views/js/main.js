...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
	//Moved Variables outside the loop and used getElementById instead of querySelectorAll
	var subContainer = document.getElementsByClassName("randomPizzaContainer");
	var dx = determineDx(subContainer[0], size);
	var newwidth = (subContainer[0].offsetWidth + dx) + 'px';
    for (var i = 0; i < subContainer.length; i++) {
      subContainer[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  var totalPix = document.body.scrollTop / 1250;
  var items = document.getElementsByClassName('mover');
    
  for (var i = 0; i < items.length; i++) {
    // document.body.scrollTop is no longer supported in Chrome.
    var phase = Math.sin(totalPix + (i % 5));
